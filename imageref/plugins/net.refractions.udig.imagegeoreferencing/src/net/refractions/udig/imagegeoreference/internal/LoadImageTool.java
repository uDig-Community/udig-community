...
final class LoadImageOnMap implements IRunnableWithProgress {
	...
	public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
        ...
        if (this.useSWT) {
			...
			try {
				...
				if (AWTImage.getColorModel() != null && 
				        !AWTImage.getColorModel().hasAlpha()) {
    ...
    private BufferedImage createTransparentAWTImage(Image image) {
        ...
        try {
            // create bufferedImage with alpha color model
	...
	private void loadMapGraphic() {
    	...
    	// ensure the mapgraphic is on the map (the layer may have been removed from the map previously
    	mapLayers = map.getMapLayers();

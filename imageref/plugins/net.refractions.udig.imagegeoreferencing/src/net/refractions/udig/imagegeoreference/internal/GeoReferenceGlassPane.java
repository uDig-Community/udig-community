...
public class GeoReferenceGlassPane extends GlassPane {
	...
	public void draw(GC graphics) {
        ...
        if (draggingImage) {
            ...
            GeoReferenceImage selectedGeoImage = GeoReferenceUtils.getSelectedGeoImage(draggingImageGraphic.getImages(), getSite().getMap());
	        ...
	        if (selectedGeoImage != null) {
	        	...
	        	graphics.drawRectangle(dragOffset.x, dragOffset.y, selectedGeoImage.getScaledWidth(), selectedGeoImage.getScaledHeight());
	        }
        }
            ...
            // get the selected geo image
            GeoReferenceImage selectedGeoImage = GeoReferenceUtils.getSelectedGeoImage(resizingImageGraphic.getImages(), getSite().getMap());
            ...
            graphics.fillOval((int)(dragOffset.x-halfsize), (int)(dragOffset.y-halfsize), PlaceMarker.DRAWING_SIZE, PlaceMarker.DRAWING_SIZE);
            //Rectangle2D stringBounds = graphics.getStringBounds(String.valueOf(count));
            //graphics.drawString(String.valueOf(count), (int)point.getX(), (int)(point.getY()+stringBounds.getHeight()), ViewportGraphics.ALIGN_MIDDLE, ViewportGraphics.ALIGN_BOTTOM);
        }
	...
	public void startResizingImage(GeoReferenceMapGraphic graphic, int x, int y) {
		...
		double br = clicked.distance(selectedGeoImage.getPosX()+selectedGeoImage.getScaledWidth(), selectedGeoImage.getPosY()+selectedGeoImage.getScaledHeight());
	...
	private Envelope getResizeImageEnvelope(GeoReferenceImage selectedGeoImage) {
		...
		// was moved the most
		double ratio = selectedGeoImage.getScaledHeight()/selectedGeoImage.getScaledWidth();
    	...
    	if (anchorCorner == 1) {
    		...
    		h = selectedGeoImage.getScaledHeight()+resizeMovement.y;
    		if (x > anchorX) x = (int)anchorX;
    		if (y > anchorY) y = (int)anchorY;
    		// ensure we aren't smaller than 10 pixels
    		if (w < 10) {
    			x = (int)anchorX - 10;
    			w = 10;

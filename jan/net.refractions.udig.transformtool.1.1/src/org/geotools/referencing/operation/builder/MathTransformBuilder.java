 ...
 * @version $Id: MathTransformBuilder.java 25050 2007-04-06 00:41:49Z jgarnett $
 * @author Jan Jezek
...
public abstract class MathTransformBuilder {
                   ...
                   MismatchedReferenceSystemException
    {
        ...
        if (points.length < necessaryNumber) {
            throw new MismatchedSizeException(Errors.format(ErrorKeys.BAD_ARGUMENT_$2,
    ...
    public Statistics getErrorStatistics() throws FactoryException {
        ...
        for (final Iterator it=getMappedPositions().iterator(); it.hasNext();) {
            ...
            try {
                ...
                throw new FactoryException(Errors.format(ErrorKeys.CANT_TRANSFORM_ENVELOPE), e);
            }
            stats.add(error);
        }
        return stats;
        ...
        if (transformation == null) {
            ...
            if (sourceBox == null) {
              ...
              //  area.intersect(new GeneralEnvelope(sourceBox));
                //try {
                    validArea = null;// new GeographicBoundingBoxImpl(area);
              //  } catch (TransformException e) {
            ...
            if (!Double.isNaN(error)) {
                final InternationalString description =
                        Vocabulary.formatInternational(VocabularyKeys.ERROR);

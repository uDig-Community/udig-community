...
public class PlaceMarkersTool extends AbstractModalTool implements ModalTool {
	...
	private static final Color[] COLORS = {Color.GREEN, Color.BLUE, Color.CYAN, Color.RED,
...
//        	action.run();
	...
	private void createNewMarkers(int number) {
		...
		if (placemarkCount > MAX_PLACEMARKERS) placemarkCount = MAX_PLACEMARKERS;
		if (placemarkCount < MIN_PLACEMARKERS) placemarkCount = MIN_PLACEMARKERS;
...
//		return new Color(red, green, blue);
    ...
    public void mousePressed(MapMouseEvent e) {
    	...
    	GeoReferenceMapGraphic mapGraphic = GeoReferenceUtils.getMapGraphic(map);
    	GeoReferenceImage selectedGeoImage = GeoReferenceUtils.getSelectedGeoImage(mapGraphic.getImages(), map);    	
    	if (selectedGeoImage == null) return;
    	...
    	if ( (addedMarkerCount+1)%2 == 1 && 
    			!(e.x >= posX && e.x <= (posX+scaledWidth)
				&& e.y >= posY && e.y <= (posY+scaledHeight)) ) {
    		return;
    	}

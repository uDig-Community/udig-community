...
public class MoveImageTool extends AbstractModalTool implements ModalTool {
    ...
    public void mouseDragged(MapMouseEvent e) {
    	...
    	if (dragging && activeGlassPane != null && selectedGeoImage != null) {
    		activeGlassPane.setDragTranslation(selectedGeoImage.getPosX()+(e.x- mouseStart.x), selectedGeoImage.getPosY()+(e.y - mouseStart.y));
			...
			try {
				...
				if (mapGraphic != null) {
					...
					if (glass instanceof GeoReferenceGlassPane) {
						...
						selectedGeoImage = GeoReferenceUtils.getSelectedGeoImage(activeMapGraphic.getImages(), getContext().getMap());
					}
...
//        }    	
    ...
    public void mouseReleased(MapMouseEvent e) {
    	...
    	if (activeMapGraphic != null) {
    		...
    		activeMapGraphic.getLayer(getContext().getMap()).refresh(null); // refresh to ensure the selected graphic reappears
    	}    	
    	
    	if (dragging) {
    	  
    		if (activeGlassPane != null && selectedGeoImage != null) {
    			activeGlassPane.endDraggingImage(selectedGeoImage.getPosX()+(e.x - mouseStart.x), selectedGeoImage.getPosY()+(e.y - mouseStart.y));

...
import net.refractions.udig.catalog.wfs.cache.WfsCachePlugin;
 ...
 * @since 1.0.0
 */
public class WFScRegistryWizardPage extends DataStoreWizardPage implements ModifyListener, SelectionListener,
UDIGConnectionPage{
...
//    protected Text pass = null;
    ...
    protected Label lblDiskLocation = null;
//    protected Label lblUseIndex = null;
//    protected Label lblPageSize = null;
    protected Button inMemory = null;
    protected Button onDisk = null;
    protected Text txtDiskLocation = null;
//    protected Text txtPageSize = null;
//    protected Button btnUseIndex = null;
    protected Button btnSelectDir = null;

    //protected Button usePullParser;
    
    protected static final String timeoutDefault = "15"; //$NON-NLS-1$
    protected static final String bufferDefault = "100"; //$NON-NLS-1$
    protected static final String defaultCacheLoc = "C:\\temp";  //$NON-NLS-1$
    /**
    ...
    private static final String WFS_WIZARD_ID = "WFSCWizard"; //$NON-NLS-1$
    private static final String WFS_RECENTLY_USED_ID = "RecentlyUsed"; //$NON-NLS-1$
    private String url = ""; //$NON-NLS-1$
    
    WFScConnectionFactory wfsConnFactory = new WFScConnectionFactory();
    public WFScRegistryWizardPage( String name ) {
        super(name);
        settings = WfsCachePlugin.getDefault().getDialogSettings().getSection(WFS_WIZARD_ID);
        if (settings == null) {
            settings = WfsCachePlugin.getDefault().getDialogSettings().addNewSection(WFS_WIZARD_ID);
        } 
    }
    
    public WFScRegistryWizardPage () {
        this(""); //$NON-NLS-1$
    }
...
//    }
    ...
    protected Map<String,Serializable> defaultParams(){
    	...
    	WFScConnectionFactory connectionFactory = new WFScConnectionFactory();
...
//        Composite composite = new Group(arg0,SWT.NULL);
        Composite composite = new Composite(arg0, SWT.NULL);
        composite.setLayout(new GridLayout(2, false));

        // add url
        Label label = new Label( composite, SWT.NONE );
        label.setText(Messages.WFScRegistryWizardPage_label_url_text ); 
        label.setToolTipText( Messages.WFScRegistryWizardPage_label_url_tooltip ); 
...
//        label.setToolTipText( Messages.WFScRegistryWizardPage_label_username_tooltip ); 
//        label.setLayoutData( new GridData(SWT.END, SWT.DEFAULT, false, false ) );
//        
...
//        label.setToolTipText( Messages.WFScRegistryWizardPage_label_password_tooltip ); 
//        label.setLayoutData( new GridData(SWT.END, SWT.DEFAULT, false, false ) );
//        
...
//      
        ...
        cache = createCacheControl(composite);
        GridData  gd = new GridData(GridData.FILL_HORIZONTAL);
        gd.horizontalSpan = 2;
        gd.verticalAlignment=2;
        cache.setLayoutData(gd);
        ...
        advancedTag.setText(Messages.WFScRegistryWizardPage_advanced_text); 
        advancedTag.setToolTipText(Messages.WFScRegistryWizardPage_advanced_tooltip); 
        ...
        gd.horizontalSpan=2;
        gd.verticalSpan=2;
        //advanced.setLayoutData( new GridData(SWT.LEFT, SWT.DEFAULT, true, true,2,1 ) );
			...
			public void run() {
				...
				if( errors!=null && !errors.isEmpty()){
					for (Map.Entry<IService, Throwable> entry : errors.entrySet()) {
						if( entry.getKey() instanceof WFScServiceImpl ){
							Throwable value = entry.getValue();
                            if( value instanceof ConnectException){
                                setErrorMessage(Messages.WFScRegistryWizardPage_serverConnectionProblem);
                            }else{
                                String message = Messages.WFScRegistryWizardPage_problemConnecting + value.getLocalizedMessage();
    ...
    private Composite createCacheControl(Composite parent){
        
        cache = new Group(parent, SWT.NONE);
        cache.setLayout(new GridLayout(3, false));
        ((Group)cache).setText(Messages.WFScRegistryWizardPage_CacheOptions);
        
        
        inMemory = new Button(cache, SWT.CHECK);
        inMemory.setSelection(true);
        inMemory.addSelectionListener(this);
        inMemory.setText(Messages.WFScRegistryWizardPage_CacheMemory);
        inMemory.setToolTipText(Messages.WFScRegistryWizardPage_CacheMemoryTooltip);
        inMemory.setLayoutData(new GridData(SWT.LEFT, SWT.DEFAULT, false, false, 3, 1 ));
        
        onDisk = new Button(cache, SWT.CHECK);
        onDisk.setSelection(false);
        onDisk.addSelectionListener(this);
        onDisk.setText(Messages.WFScRegistryWizardPage_CacheDisk);
        onDisk.setToolTipText(Messages.WFScRegistryWizardPage_CacheDiskTooltip);
        onDisk.setLayoutData(new GridData(SWT.LEFT, SWT.DEFAULT, false, false, 3, 1 ));
        
        this.lblDiskLocation = new Label(cache, SWT.NONE);
        this.lblDiskLocation.setEnabled(false);
        this.lblDiskLocation.setText(Messages.WFScRegistryWizardPage_DiskCacheLocation);
        this.lblDiskLocation.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false ));

        this.txtDiskLocation = new Text(cache, SWT.BORDER);
        this.txtDiskLocation.setEnabled(false);
        this.txtDiskLocation.setText(defaultCacheLoc);
        this.txtDiskLocation.setLayoutData(new GridData(SWT.FILL, SWT.DEFAULT, true, false));
        
        btnSelectDir = new Button(cache, SWT.PUSH);
        btnSelectDir.setText("..."); //$NON-NLS-1$
        final WFScRegistryWizardPage pg = this;
        btnSelectDir.addSelectionListener(new SelectionListener(){
            ...
            public void widgetSelected( SelectionEvent e ) {
                DirectoryDialog dd = new DirectoryDialog(pg.getShell());
                dd.setFilterPath(txtDiskLocation.getText());
                String result = dd.open();
                if (result != null){
                    txtDiskLocation.setText(result);
                }
            }});
        
        new Label(cache, SWT.NONE);
    ...
    private Composite createAdvancedControl( Composite parent ) {
        ...
        label.setToolTipText( Messages.WFScRegistryWizardPage_label_post_tooltip ); 
        label.setLayoutData( new GridData(SWT.RIGHT, SWT.DEFAULT, false, false ) );
        
        postDefault = new Button( advanced, SWT.CHECK );
        postDefault.setLayoutData( new GridData(SWT.LEFT, SWT.DEFAULT, true, false ) );
        ...
        // buffer
        label = new Label( advanced, SWT.NONE );
        label.setText(Messages.WFScRegistryWizardPage_label_buffer_text); 
        label.setToolTipText( Messages.WFScRegistryWizardPage_label_buffer_tooltip ); 
        label.setLayoutData( new GridData(SWT.RIGHT, SWT.DEFAULT, false, false) );
        
        bufferText = new Text( advanced, SWT.BORDER | SWT.LEFT );
        ...
        // timeout
        label = new Label( advanced, SWT.NONE );
        label.setText(Messages.WFScRegistryWizardPage_label_timeout_text); 
        label.setToolTipText( Messages.WFScRegistryWizardPage_label_timeout_tooltip ); 
        label.setLayoutData( new GridData(SWT.RIGHT, SWT.DEFAULT, false, false) );
        
        timeoutText = new Text( advanced, SWT.BORDER | SWT.LEFT );
        ...
        label = new Label( advanced, SWT.NONE );
        label.setText(Messages.WFScRegistryWizardPage_label_maxfeat); 
        label.setToolTipText( Messages.WFScRegistryWizardPage_label_maxfeat_tooltip ); 
        label.setLayoutData( new GridData(SWT.RIGHT, SWT.DEFAULT, false, false) );

        maxFeaturesText = new Text( advanced, SWT.BORDER | SWT.LEFT );
    ...
    public void widgetSelected( SelectionEvent e ) {
		...
		} else if (b.equals(postDefault)) {
		    if (postDefault.getSelection() && getDefault.getSelection()) {
		        getDefault.setSelection(false);
			}
		} else 	if (b.equals(advancedTag)) {
			advanced.setVisible(advancedTag.getSelection());
		//	cache.setVisible(advancedTag.getSelection());
			
		
		} else if (b.equals(inMemory) || b.equals(onDisk)){
		    if (b.equals(inMemory) && onDisk.getSelection()){
		        onDisk.setSelection(false);
		    }else if (b.equals(onDisk) && inMemory.getSelection()){
		        inMemory.setSelection(false);
		    }
		    boolean isenabled = onDisk.getSelection();
		    this.lblDiskLocation.setEnabled(isenabled);
//		    this.lblPageSize.setEnabled(isenabled);
//		    this.lblUseIndex.setEnabled(isenabled);
		    
		    this.txtDiskLocation.setEnabled(isenabled);
//		    this.txtPageSize.setEnabled(isenabled);
//		    this.btnUseIndex.setEnabled(isenabled);
		    this.btnSelectDir.setEnabled(isenabled);
		}
...
//        }
        ...
        if(!WFScRegistryWizardPage.timeoutDefault.equals(timeout)){
            // parse the string
            Integer sec = 1;
            try{
            ...
            dsParams.put(WFSDataStoreFactory.TIMEOUT.key,sec*1000);
        }
        
        String buffer = bufferText.getText();
        if(!WFScRegistryWizardPage.bufferDefault.equals(buffer)){
            // parse the string
            Integer sec = null;
            try{
        ...
        if(!WFScRegistryWizardPage.maxFeaturesDefault.equals(timeout)){
            // parse the string
            Integer max = null;
            try{
        ...
        if (inMemory.getSelection()){
            dsParams.put(WFScServiceImpl.CACHE_TYPE_KEY, WFScServiceImpl.CACHE_MEMORY);
        }else if (onDisk.getSelection()){
            dsParams.put(WFScServiceImpl.CACHE_TYPE_KEY, WFScServiceImpl.CACHE_DISK);
         
            String dir = txtDiskLocation.getText() + File.separator;            
            dsParams.put(WFScServiceImpl.CACHE_DIR_KEY, dir);
        }else{
            dsParams.put(WFScServiceImpl.CACHE_TYPE_KEY, WFScServiceImpl.CACHE_NONE);
        }

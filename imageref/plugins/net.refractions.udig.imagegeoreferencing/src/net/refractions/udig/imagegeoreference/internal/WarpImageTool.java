...
public class WarpImageTool extends AbstractModalTool implements ModalTool {
    ...
    private void warpImage(IMap map) {
        ...
        if (warpedCoverage != null) {
            ...
            if (filename != null && filename != "") { //$NON-NLS-1$
                ...
                if (images != null) images.remove(selectedGeoImage.getFilename());
            }
            
            // refresh the map  
            if (mapGraphic != null) mapGraphic.getLayer(map).refresh(null);                 
        }
    }    
    ...
    private boolean validateMarkers() {
        if (imageMarkers == null || imageMarkers.size() < PlaceMarkersTool.MIN_PLACEMARKERS ||
                basemapMarkers == null || basemapMarkers.size() < PlaceMarkersTool.MIN_PLACEMARKERS ||
                imageMarkers.size() != basemapMarkers.size()) {
            return false;
        }
    ...
    private Point2D[] getImagePoints(GeoReferenceImage selectedGeoImage, List<PlaceMarker> markers) {
        ...
        for (PlaceMarker marker : markers) {
            ...
            java.awt.Point fullPoint = new java.awt.Point( (int)(image.getWidth(null)*xScale), (int)(image.getHeight(null)*yScale) );
...
final class WarpImageProcess implements IRunnableWithProgress {
    ...
    public WarpImageProcess(IMap map, GeoReferenceImage geoImage, Point2D[] srcCoords,
            Point2D[] dstCoords, int warpValue) {
        this.map = map;
    ...
    public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
        ...
        DefaultDerivedCRS derivedCRS = 
            new DefaultDerivedCRS("imageCRS",  crs, warpTransform, //$NON-NLS-1$
        ...
        ReferencedEnvelope ref = new ReferencedEnvelope(0, 
                image.getWidth(null), 0, image.getHeight(null), derivedCRS);
...
//          IGeoResource resource = CatalogPlugin.getDefault().getLocalCatalog().createTemporaryResource(tempfile);  
...
final class WriteTifAndLoadProcess implements IRunnableWithProgress {
    ...
    public WriteTifAndLoadProcess(IMap map, GridCoverage2D warpedCoverage,
            String filename) {
        this.map = map;
